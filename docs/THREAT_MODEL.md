...
| T02 | Mainnet misconfiguration | Real-fund loss | Hard assertion `chainId === 50312` in environment, policy, signer, and receipt; no mainnet write mode or address fallback | Test that a non-50312 chain cannot construct signer request |
...
| T14 | Nonce race and ambiguous retry | Duplicate or replaced transaction | One-shot worker uses an exclusive signer lock, durable one-time bundle claim and fsynced hash-chain journal. Post-boundary ambiguity retains the lock and disables retry. Residual: SDK path does not expose explicit nonce/raw transaction persistence, so recovery is manual and single-replica only | 25 concurrent journal appends and duplicate/lock tests pass; process-kill/raw-tx recovery remains open |
...
| T22 | Worker crash between stages | Duplicate writes/incomplete audit | Durable pre-submission claim and append-only stage journal; ambiguous failures retain the signer lock and never auto-resubmit. Operator must reconcile nonce and chain state | Restart integrity/corrupt/torn-record tests pass; kill-at-every-boundary integration test remains open |
