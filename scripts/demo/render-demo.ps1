...
try {
  $subtitleFilter = "subtitles='$subtitleInput':force_style='FontName=Arial,FontSize=10,PrimaryColour=&H00FFFFFF,OutlineColour=&H00131E1A,BorderStyle=1,Outline=1.4,Shadow=0,MarginR=34,MarginV=28,Alignment=3'"
  & ffmpeg -y -hide_banner -loglevel warning -i $videoInput -i $audioInput -filter_complex "[0:v]trim=0:155,setpts=PTS-STARTPTS,$subtitleFilter[v];[1:a]atrim=0:155,asetpts=PTS-STARTPTS,loudnorm=I=-16:LRA=7:TP=-1.5,aresample=48000[a]" -map "[v]" -map "[a]" -c:v libx264 -preset slow -crf 18 -pix_fmt yuv420p -r 30 -c:a aac -b:a 192k -movflags +faststart -t 155 $videoOutput
