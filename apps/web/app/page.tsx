...
import type { ExecutionBundle, HedgePlan, OutcomeGuardReceipt, PolicyResult, EventMarketSnapshot } from "@outcome-guard/schemas";
...
export default function Home() {
  ...
  const authorize = async () => {
    if (!data) return;
    if (data.mode !== "live") { setError("Derive a fresh live plan before signing an intent."); return; }
  ...
  return <main>
    ...
    <section id="command-center" className="workspace" aria-label="Protection command center">
      ...
      <div className="results">
        ...
        <section className="authorize card"><div><span>EXACT INTENT REVIEW · NO TRANSACTION</span><h2>Buy {data?.plan.normalizedShares.toFixed(3) ?? "—"} DOWN shares · IOC</h2><div className="orderReview"><span><small>Chain</small>Shannon · 50312</span><span><small>Maximum premium</small>{money(maxPremium)} {data?.market.collateral.symbol}</span><span><small>Worst price</small>{data ? `${(data.plan.worstPrice * 100).toFixed(2)}¢` : "—"}</span><span><small>Market</small>{data ? short(data.market.marketId) : "—"}</span><span><small>Expiry</small>{data ? new Date(data.market.expiry).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "—"}</span><span><small>Snapshot seal</small>{data ? short(data.receipt.integrity.digest) : "—"}</span></div><p>Signing binds this live snapshot and intent. It does not submit a transaction. Fresh gas, total-risk, book, status and receipt checks remain mandatory before a separate transaction signature.</p></div><div className="authorizationActions"><button onClick={authorize} disabled={loading || data?.mode !== "live" || authorizableFailures.length > 0}>{signature ? "Intent signed ✓" : data?.mode !== "live" ? "Derive live plan first" : "Review & sign intent"}</button>{executionBundle && <button className="secondaryAction" onClick={downloadBundle}>Download signed bundle</button>}</div></section>
