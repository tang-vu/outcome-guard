  ...
  return <main>
    ...
    <section id="command-center" className="workspace" aria-label="Protection command center">
      ...
      <div className="results">
        ...
        <section className="card settledReplay"><div className="cardHead"><div><span>VERIFIED OWNED LIFECYCLE · REAL SHANNON EXECUTION</span><h2>{settledReplay?.market.question ?? "Loading linked execution and settlement evidence…"}</h2></div><b>{settledReplay?.terminalState.status ?? "READING"}</b></div>{settledReplay && <><div className="replayFlow"><div><small>Bounded IOC</small><strong>{settledReplay.execution.filledSize} {settledReplay.execution.filledOutcome}</strong><a href={settledReplay.execution.explorerUrl} target="_blank" rel="noreferrer">Open transaction ↗</a></div><i>→</i><div><small>Reconciled position</small><strong>{settledReplay.position.amount} {settledReplay.position.outcome}</strong><span>{short(settledReplay.market.marketId)}</span></div><i>→</i><div className="replayOutcome"><small>Winning outcome</small><strong>{settledReplay.terminalState.winningOutcome}</strong><span>{settledReplay.terminalState.status}</span></div><i>→</i><div className="notClaimed"><small>Claimable payout</small><strong>{settledReplay.terminalState.claimable} tUSDC</strong><span>{settledReplay.terminalState.claimable === "0" ? "Held outcome lost · no redemption" : "Separate redemption authorization required"}</span></div></div><div className="replaySeal"><span>✓ Execution → settlement digest link verified</span><a href={`/receipts/${settledReplay.settlementReceiptDigest}`}><code>{settledReplay.settlementReceiptDigest}</code></a></div><p><b>Evidence boundary:</b> this was a real dedicated-test-agent execution, not a human-wallet trade. The DOWN protection expired without payout because the market resolved UP; OutcomeGuard records the premium-at-risk case instead of fabricating a claim.</p></>}</section>
      </div>
    </section>
